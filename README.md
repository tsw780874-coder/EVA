...
EVA（E-Commerce Value Analyst）是一个可信赖的 AI 电商购物智能决策系统。用户以自然语言描述购物需求，系统通过 RAG 知识库检索真实数据，结合多 LLM 并行竞速，自动完成意图分析、商品搜索、价格对比和购买建议的全链路智能决策。

> **v5 核心升级：** RAG 优先架构，反幻觉体系，数据验证层，引用机制，可信度评分。

## 架构概览

```
用户 → Next.js 前端 → FastAPI 后端 → RAG-First Agent 管线
                            │
        ┌───────────────────┼───────────────────┐
        ▼                   ▼                   ▼
...
### RAG-First Pipeline (v5)

```
User → Intent → RAG Search → Data Verification → LLM Summarize → Citations → Report
                       │              │
                  Hybrid Search   Confidence Score
                  (BM25+Vector)    (0-100%)
```
...
| 可信度 | RAG 知识库，混合搜索 (BM25+Milvus)，数据验证层，引用追踪，可信度评分 |
...
│   ├── app/                     # 页面路由（App Router）
│   │   ├── assistant/           # 主对话页面
│   │   ├── admin/               # 管理后台（模型/RAG/MCP/日志）
│   │   ├── login/register/      # 认证页面
│   │   └── product/reports/     # 功能页面
│   ├── components/              # 公共组件
│   ├── stores/                  # Zustand 状态管理
│   └── lib/                     # API 客户端 (SSE 流式)
│
...
│   ├── app/
│   │   ├── agent/               # Agent 核心
│   │   │   ├── pipeline.py      # RAG-First 主管线 (v5)
│   │   │   ├── model_router.py  # 智能模型路由
│   │   │   ├── llm_utils.py     # LLM 调用（竞速+流式+缓存）
│   │   │   ├── product_templates.py  # 模板数据库（标记 simulated）
│   │   │   └── intent.py        # 意图分类 (<1ms)
│   │   ├── api/v1/              # REST API 路由
...
│   │   │   ├── database.py      # 异步数据库引擎
│   │   │   ├── security.py      # JWT 认证
│   │   │   ├── verifier.py      # 数据验证层 (v5)
│   │   │   ├── citations.py     # 引用追踪 (v5)
│   │   │   ├── confidence.py    # 可信度评分 (v5)
│   │   │   └── perf.py          # 性能计时器
│   │   └── cache/
│   │       └── redis_cache.py   # Redis+内存双层缓存
│   ├── rag/                     # RAG 知识库管道
│   │   ├── hybrid_search.py     # BM25 + 向量混合搜索 (v5)
...
反幻觉核心原则：
1. **禁止 LLM 生成商品数据** — 商品信息来自 RAG/数据库
2. **所有事实声明必须可验证** — 数据验证层交叉检查
3. **无数据时明确告知** — "未找到可靠信息"，绝不猜测
4. **每项数据标注来源** — 引用追踪器自动生成
5. **可信度透明** — 0-100% 评分，<70% 警告用户

```
RAG Search → 数据验证 → LLM 仅做总结 → 引用 + 可信度 → 报告
```
